    ...
    public class YSTB
    {
        ...
        public List<string> ExportString()
        {
            ...
            foreach (var code in Instructions)
            {
                ...
                else if (code.Meta.Name == "_" && code.Arguments.Length == 1 && code.Meta.Arguments[0].WTF == 0x03 && code.Arguments[0].EnsurePureString())
                {
                ...
                else if (code.Meta.Name == "GOSUB" && code.Arguments.Length >= 2 && code.Arguments[0].EnsurePureString())
                {
                    ...
                    case "es.char.name.mark.set":
                        ...
                        {
                            result.Add(code.Arguments[1].GetString());
                        }
                        break;
                    }
                }
        ...
        public int Patch(List<string> patch)
        {
            ...
            foreach (var code in Instructions)
            {
                ...
                else if (code.Meta.Name == "_" && code.Arguments.Length == 1 && code.Meta.Arguments[0].WTF == 0x03 && code.Arguments[0].EnsurePureString())
                {
                    if (patch[i] != null)
                    {
                        code.Arguments[0].LoadString(patch[i]);
                    }
                    if (++i >= patch.Count)
                    {
                        break;
                ...
                else if (code.Meta.Name == "GOSUB" && code.Arguments.Length >= 2 && code.Arguments[0].EnsurePureString())
                {
                    ...
                    case "es.char.name.mark.set":
                        if (code.Arguments[1].EnsurePureString())
                        {
                            if (patch[i] != null)
                            {
                                code.Arguments[1].LoadString(patch[i]);
                            }
                            if (++i >= patch.Count)
                            {
                                break;
