...
namespace YuRIS
{
    public static class CheckSum
    {
        ...
        private static readonly uint[] Magic32_Table = new uint[]
        {
        ...
        // Algorithm name unknown, found in DL version XOR key calculation
        public static uint Magic32(string data, int len = -1)
        {
            int index = 0;
            uint a, b, c, d = 0xFFFFFFFF;
            ...
            if (high != 0)
            {
                while (index < high)
                {
                    a = Magic32_Table[(byte)(d ^ data[index++])] ^ (d >> 8);
                    b = Magic32_Table[(byte)(a ^ data[index++])] ^ (a >> 8);
                    c = Magic32_Table[(byte)(b ^ data[index++])] ^ (b >> 8);
                    d = Magic32_Table[(byte)(c ^ data[index++])] ^ (c >> 8);
                }
            }
            int low = len & 3;
            if (low != 0)
            {
                for (int i = 0; i < low; i++)
                {
                    d = Magic32_Table[(byte)(d ^ data[index++])] ^ (d >> 8);
                }
            }
            return ~d;
        }
